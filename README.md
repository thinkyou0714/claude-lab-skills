...
## Plugin 一覧（本リポジトリ収録: 6 プラグイン / 43 スキル）
...
| [lab-data-auth-ops](./lab-data-auth-ops/) | データ設計・認証境界・PII・監査ログ・アクセス制御 | 8 | `/data-review` |
...
導入できるプラグイン: `lab-thinking-core` / `lab-strategy-design` / `lab-automation-architecture` / `lab-data-auth-ops` / `lab-implementation-flow` / `lab-communication-translation`
...
| Phase 7 | lab-strategy-design（7）・lab-data-auth-ops（6）本実装 → 計 41 スキル（6 プラグイン） | 完了 |
| Phase 8 | lab-data-auth-ops に `auth-system-design` / `secret-management-review` を追加 → 6 プラグイン / 43 スキル | 完了 |
