...
## Plugin 一覧（本リポジトリ収録: 4 プラグイン / 27 スキル）
...
以下は設計済みだが、本公開リポジトリには**未収録**のプラグイン。実体が存在しないため、
ドキュメント上で「収録済み」とは扱わない（参照リンクも張らない）。

| Plugin | 責務 | 状態 |
|---|---|---|
| `lab-strategy-design` | 目標検証・代替案比較・スコープ設計・実行中戦略の査定 | 計画中（未収録） |
| `lab-data-auth-ops` | データ設計・認証境界・ログ・PII | 計画中（未収録） |
...
| Phase 2 | lab-automation-architecture 骨組み（lab-data-auth-ops は Roadmap・未収録） | 完了 |
